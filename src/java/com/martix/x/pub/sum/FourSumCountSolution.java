 ...
 * <p>
 * 为了使问题简单化，所有的 A, B, C, D 具有相同的长度N，且 0 ≤ N ≤ 500 。所有整数的范围在 -228 到 228 - 1 之间，最终结果不会超过 231 - 1 。
 * <p>
 * 输入:
 * A = [ 1, 2]
 * B = [-2,-1]
...
public class FourSumCountSolution {
    ...
    /**
     * 对于A 和B，我们使用二重循环对它们进行遍历，
     * 得到所有A[i]+B[j] 的值并存入哈希映射中。
     * 对于哈希映射中的每个键值对，每个键表示一种A[i]+B[j]，对应的值为A[i]+B[j] 出现的次数。
     * 对于C 和D，我们同样使用二重循环对它们进行遍历。
     * 当遍历到C[k]+D[l] 时，如果−(C[k]+D[l]) 出现在哈希映射中，
